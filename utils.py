...
def plot_auc(labels, preds, savename="MLP",classnames=['a','b','c']):
    save_path = os.path.join(dir,'{}_ROC_AUC.png'.format(savename))
    n_classes = labels.shape[1]
...
def bi_classify_metrics(labels, preds, plot_cm=False, savename='MLP',classnames=['Negative','Positive']):
    ...
    """
    save_path = os.path.join(dir,'{}_confusion_matrix.png'.format(savename))
    if len(list(labels.shape)) == 1:
        y_true = labels
    ...
    mcc = (tp*tn - fp*fn) / ((tp+fp)*(fn+tp)*(fn+tn)*(fp+tn))**0.5 # equal: matthews_corrcoef(y_true, y_pred)
...
def multi_classify_metrics(labels, preds, average_method='macro', plot_auc_curve=False, plot_confuse_matrix=False, savename="MLP",classnames=['a','b','c']):
    """
    args:
